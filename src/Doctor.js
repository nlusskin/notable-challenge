    ...
    <td>
      <div id="doctorListItem">
        ...
        <div id='dContainer'>
          ...
          <div id='dAction'>
            <button onClick={() => confirmRemove(p)}>&#10004;</button><br></br>
    ...
    <modal>
      ...
      <div id='modal'>
        <div style={{width: '100%', height: '60px'}}>
          <img src={'/' + imgs[tk[p.i.key]]} />
          <button onClick={() => confirmRemove(p)}>&#10004;</button>
...
function confirmRemove(p) {
  let c = window.confirm("Are you sure you want to remove the patient from the queue?")
  if (c){
...
class DoctorView extends React.Component {
  ...
  constructor(p) {
    super(p)
    this.state = {listItems: [], descModal: {show: false}}
  ...
  render() {
      return (
        <div id='doctorListContainer'>
          <table><tr>
          {(this.state.listItems.length == 0) ? <label>No Patients Currently Waiting</label> : <div><label>Current Patient:</label> <br></br> {this.state.listItems.map(v => <List i={v} c={this} /> )}</div>}
          </tr></table>
